...
class RecoveryDecision(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "recovery_case_id",
            "case_state_version",
            name="uq_recovery_decision_case_state_version",
        ),
        CheckConstraint(
            "recovery_probability >= 0 "
            "AND recovery_probability <= 1",
            name="recovery_decision_probability_range",
        ),
        CheckConstraint(
            "expected_recovery_rupees >= 0",
            name="recovery_decision_expected_recovery_non_negative",
        ),
        CheckConstraint(
            "estimated_action_cost_rupees >= 0",
            name="recovery_decision_action_cost_non_negative",
        ),
        Index(
    ...
    final_action: Mapped[RecoveryActionType | None] = mapped_column(
        recovery_action_enum,
        nullable=True,
    )
    ...
    status: Mapped[RecoveryDecisionStatus] = mapped_column(
        SqlEnum(
            RecoveryDecisionStatus,
    ...
    expected_recovery_rupees: Mapped[Decimal] = mapped_column(
        Numeric(14, 2),
        nullable=False,
    )

    estimated_action_cost_rupees: Mapped[Decimal] = mapped_column(
        Numeric(14, 2),
        nullable=False,
        default=Decimal("0.00"),
    )

    expected_net_value_rupees: Mapped[Decimal] = mapped_column(
        Numeric(14, 2),
        nullable=False,
    )
    ...
    decision_inputs: Mapped[dict[str, Any]] = mapped_column(
        JSONB,
        nullable=False,
        default=dict,
    )
